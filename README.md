...
Guest mode needs no API keys. Search uses a recorded SuggestAPI-shaped
catalog on disk (`storefront/data/recorded-responses.json`) — it does not
call mock.shop for products. Add-to-cart still creates a Shopify cart
against `https://mock.shop/api` so checkout has a real handoff URL.
...
2. Hover a result and **Add to cart**
3. **Checkout on Shopify** — that opens mock.shop with those lines in the cart.
...
Browser  →  local shopper API
                ...
                └─ Shopify cart permalink            (store cart + checkout)
```

- Empty `SUGGESTAPI_PUBLIC_KEY` → local catalog file (no mock.shop product fetch).
